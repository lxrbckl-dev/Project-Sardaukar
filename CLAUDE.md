...
- **Read-only** access to **SITMAP** (Alex's portfolio board at `lxrbckl-labs/projects/2`) — one card per `lxrbckl-labs` repo. Alex drives it; TPM only writes to backfill a brand-new repo in Backlog and notify him. See `.claude/agents/tpm-agent.md` for the full spec.
...
- **SITMAP** (`lxrbckl-labs/projects/2`) — a separate portfolio-altitude board with **one card per `lxrbckl-labs` repo** (not per issue). **READ-ONLY for agents** — Alex drives it; TPM only writes to backfill a brand-new repo in Backlog. `lxrbckl-labs` only — other orgs (e.g. t5-labs) do not appear here. If Alex asks to "map the work" / "update the board" without naming SITMAP, he means the per-org KanBan, not SITMAP. Full spec in `.claude/agents/tpm-agent.md`.
...
| Portfolio tracking | SITMAP board at `lxrbckl-labs/projects/2`, read-only for agents | Alex's portfolio scoreboard; agents don't write there (except brand-new-repo backfill) so the view stays his |
